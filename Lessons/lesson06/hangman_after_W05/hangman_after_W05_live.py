...
class HangmanGame:
    ...
    def load_words_from_file(self, target_path=None):
        ...
        with open(target_path, "r", encoding="utf-8") as file:
            ...
            # in mängder av anrop.
            self.possible_words = [x.strip() for x in file]
...
def main():
    wordlist_input = (input("Skriv in namnet på en egen ordlista, med .txt, eller lämna tomt: ")
                      or None)
    game = HangmanGame(wordlist_input)
    done = False
    while done != "":
        game.setup()
        game.game_loop()
